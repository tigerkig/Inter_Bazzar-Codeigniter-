                                    ...
                                    <table class="table table-orders">
                                        ...
                                        foreach ($order_products as $item) :
                                            if ($item->seller_id == $this->auth_user->id) :
                                                $sale_subtotal += currency_convert($item->product_unit_price,$item->product_currency, 'EUR') * $item->product_quantity;
                                                $sale_vat += currency_convert($item->product_vat,$item->product_currency, 'EUR');
                                                $sale_shipping += currency_convert($item->product_shipping_cost,$item->product_currency, 'EUR');
                                                $sale_total += currency_convert($item->product_total_price,$item->product_currency, 'EUR'); 
                                                // currency_convert($item->product_total_price, $item->product_currency, 'EUR')
                                                ?>
                                                <tr>
                                                     <td style="width: 50%">
                                                        <div class="table-item-product">
                                                            ...
                                                            <div class="right">
                                                                ...
                                                                <p><span class="span-product-dtl-table"><?php echo trans("unit_price"); ?>:</span>
                                                                     <?php echo price_formatted(currency_convert($item->product_unit_price, $item->product_currency, 'EUR'), 'EUR') ?> (<?php echo price_formatted($item->product_unit_price, $item->product_currency); ?>)
                                                                    </p>
                                                                <p><span class="span-product-dtl-table"><?php echo trans("quantity"); ?>:</span><?php echo $item->product_quantity; ?></p>
                                                                <?php if ($item->product_type == 'physical') : ?>
                                                                    <p><span class="span-product-dtl-table"><?php echo trans("shipping"); ?>:</span>
                                                                         <?php echo price_formatted(currency_convert($item->product_shipping_cost, $item->product_currency, 'EUR'), 'EUR') ?> (
                                                                        <?php echo price_formatted($item->product_shipping_cost, $item->product_currency); ?>)</p>
                                                                <?php endif; ?>
                                                                <?php if (!empty($item->product_vat)) : ?>
                                                                    <p><span class="span-product-dtl-table"><?php echo trans("vat"); ?>&nbsp;(<?php echo $item->product_vat_rate; ?>%):</span>

                                                                         <?php echo price_formatted(currency_convert($item->product_vat, $item->product_currency, 'EUR'), 'EUR') ?> (<?php echo price_formatted($item->product_vat, $item->product_currency); ?> )</p>
                                                                    <p><span class="span-product-dtl-table"><?php echo trans("total"); ?>:</span>
                                                                         <?php echo price_formatted(currency_convert($item->product_total_price, $item->product_currency, 'EUR'), 'EUR') ?>( <?php echo price_formatted($item->product_total_price, $item->product_currency); ?>)
                                                                       </p>
                                                                <?php else : ?>
                                                                    <p><span class="span-product-dtl-table"><?php echo trans("total"); ?>:</span>
                                                                         <?php echo price_formatted(currency_convert($item->product_total_price, $item->product_currency, 'EUR'), 'EUR') ?> (<?php echo price_formatted($item->product_total_price, $item->product_currency); ?>)
                                                    ...
                                                    <td style="width: 25%">
                                                        <?php if ($item->order_status == "completed" || $item->order_status == 'refund_completed' || $item->order_status == 'refund_cancelled')  : ?>
                                                            <strong class="font-600"><i class="icon-check"></i>&nbsp;<?php echo trans("approved"); ?></strong>

                                                        <?php elseif ($item->order_status == 'refund_pending'): ?>
                                                            <?php echo form_open('refund-approve'); ?>
                                                            <button type="submit" class="mt-2 btn btn-sm btn-custom btn-block"><i class="icon-check"></i><?php echo trans("confirm_refund_received"); ?></button>
                                                            <input type="hidden" name="order_product_id" value="<?php echo $item->id; ?>">
                                                            <?php echo form_close(); ?>

                                                            <?php echo form_open('refund-cancel'); ?>
                                                            <button type="submit" class="mt-2 btn btn-sm btn-custom btn-block"><?php echo trans("cancel_refund"); ?></button>
                                                            <input type="hidden" name="order_product_id" value="<?php echo $item->id; ?>">
                                                            <?php echo form_close(); ?>
                                                        <?php else : ?>
                                                            <p class="m-b-5">
                                ...
                                <div class="order-total">
                                    <div class="row">
                                        ...
                                        <div class="col-6 col-right">
                                            <strong class="font-600">
                                                <?php echo price_formatted($sale_subtotal, $order->price_currency); ?>
                                                <?php // echo price_formatted($order->price_subtotal, $order->price_currency); ?>
                                            </strong>
                                        </div>
                                    </div>
                                    <?php if (!empty($sale_vat)) : ?>
                                        <div class="row">
                                            ...
                                            <div class="col-6 col-right">
                                                <strong class="font-600">
                                                    <?php echo price_formatted($sale_vat, $order->price_currency); ?>
                                                    </strong>
                                            </div>
                                        </div>
                                    <?php endif; ?>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-6 col-right">
                                            <strong class="font-600"><?php echo price_formatted($sale_total, $order->price_currency); ?>
                                            </strong>
                                        </div>
                                    </div>
                                </div>
