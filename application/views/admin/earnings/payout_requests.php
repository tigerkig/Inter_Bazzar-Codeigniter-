...
<div class="box" style="position:absolute;">
	<div class="box-header with-border">
		...
		<div class="right">
			<a href="<?php echo admin_url(); ?>add-payout" class="btn btn-primary btn-add-new">
			...
			<div class="col-sm-12">
				<div class="table-responsive">
					<table class="table table-bordered table-striped" role="grid">
						...
						<tbody>
							<?php foreach ($payout_requests as $key => $item): ?>
								...
								<tr>
									...
									<td>
									<?php echo ucwords(str_replace('_',' ',$item->payout_method)); ?>

										<p class="m-0">
											<!-- <button type="button" class="btn btn-xs btn-primary" onclick="payout_modal(<?php echo $value->id?>)" data-toggle="modal" data-target="#accountDetailsModel_<?php echo $item->id; ?>" data-payment="<?php echo $item->payout_method; ?>"><?php echo trans("see_details"); ?></button> -->
											<button type="button" class="btn btn-xs btn-primary" onclick="payout_settings_modal(<?php echo $item->id?>)" data-toggle="modal" data-target="#accountDetailsModel" data-payment="<?php echo $item->payout_method; ?>"><?php echo trans("see_details"); ?></button>
										</p>
									</td>
		...
		<div class="modal-dialog">
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title"><?php echo ($item->payout_method); ?></h4>
				</div>
					...
					<?php if (!empty($payout)): ?>
						...
						<?php else: ?>
							<div class="row">
								...
								<div class="col-sm-8">
									<strong id="other_email">
										&nbsp;<?php echo get_payout_request_details($item->payout_method,$item->user_id,'email'); ?>
									</strong>
								</div>
							</div>
							<div class="row">
								...
								<div class="col-sm-8">
									<strong id="other_phone">
										&nbsp;<?php echo get_payout_request_details($item->payout_method,$item->user_id,'phone'); ?>
									</strong>
								</div>
							</div>
						<?php endif; ?>
...
<script>
	$('doucment').ready(function(){
		$('#accountDetailsModel').on('hidden.bs.modal', function () {
			$(' #modal_table tbody').html('');

		})
	})
	function payout_settings_modal(id) {
		console.log(id);
        var data = {
            "id": id
        };
        ...
        $.ajax({
            type: "POST",
            url: base_url + "ajax_controller/get_payout_modal_information",
            data: data,
            success: function (response) {
				var obj = JSON.parse(response);	
				console.log("response");
				for (let index = 0; index < obj.name.length; index++) {
					var i = index+1;
					$(' #modal_table tbody').append([
						"<tr>",
							"<td class='location'>" + i + " </td>",
							"<td class='location'>" + obj.name[index] + " </td>",
							"<td class='location'>" + obj.content[index] + " </td>",
						"</tr>",
					].join(''));
...
<div id="accountDetailsModel" class="modal fade in" role="dialog">
	<div class="modal-dialog" style="right: -8%;">
		<!-- Modal content-->
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
			...
			<div class="modal-body">
				<table class="table table-bordered table-striped" role="grid" id="modal_table">
					<thead>
						<tr>
							<th class="col-md-2 location" >No</th>
							<th class="col-md-5 location" >Name</th>
							<th class="col-md-5 location" >Value</th>
						</tr>
					</thead>
