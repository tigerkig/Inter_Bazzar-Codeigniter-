...
class Ajax_controller extends Home_Core_Controller
{
    ...
    */
    ...
    public function get_payout_modal_information()
    {
        $array = [];
        $name = [];
        $content = [];
        ...
        $this->db->where('id', $id);
        $this->db->select('input_data_new');
        $this->db->select('set_input_data_new');
        $query = $this->db->get('payouts')->row();
        foreach ($query as $key => $value) {
            if($key == "set_input_data_new"){
                foreach (json_decode($value) as $key_1 => $child) {
                    $number = preg_replace('/[^0-9_]/', '', $key_1);
                    $array[] = $number;
                }
                foreach (json_decode($value) as $key_1 => $child) {
                    $number = preg_replace('/[^0-9_]/', '', $key_1);
                    if($key_1 == 'select'.$number){
                        $content[] = $child;
                    }
                }
            }
        ...
        foreach ($query as $key => $value) {
            if($key == "input_data_new"){
                foreach (json_decode($value) as $key_1 => $child) {
                    for ($i=0; $i < count($array) ; $i++) { 
                        if($key_1 == 'table_name'.$array[$i]){
                            $name[] = $child;
                        }
                    }
                }
    ...
    */
    ...
    {
        $input_value = $this->input->get('input_value', true);
        $input_value = remove_special_characters($input_value);
        $data = array(
            'result' => 0,
            'response' => ''
        ...
        if (!empty($input_value)) {
            $countries = $this->location_model->search_countries_by_name($input_value);
    ...
    {
        $orders = $this->order_admin_model->get_orders_notification_count();
...
//get Order Notification
    public function refunds_p_notification()
    {

        $this->db->select('refunds.id');
    ...
    //get Order Notification
    public function refunds_c_notification()
    {
        $this->db->select('refunds.id');
        $this->db->join('order_products','refunds.refund_order_product_id = order_products.id');
        $this->db->from('refunds');
        $this->db->where('order_products.order_status','refund_completed');
        $num_results = $this->db->count_all_results();
    ...
    //get Order Notification
    public function refunds_can_notification()
    {
        $this->db->select('refunds.id');
        $this->db->join('order_products','refunds.refund_order_product_id = order_products.id');
        $this->db->from('refunds');
        $this->db->where('order_products.order_status','refund_cancelled');
        $num_results = $this->db->count_all_results();
    ...
    //get Bank Transfer Notification
    public function get_bank_transfer_notification()
    {
        $bank_transfer = $this->order_admin_model->bank_transfer_notification_count();
    ...
    public function order_complete_bank_notification()
    {
        $order_complete = $this->order_admin_model->order_complete_bank_notification_count();
    ...
    public function payout_request_notification()
    {
        $payout = $this->earnings_admin_model->payout_request_notification_count();
        if(!empty($payout)){
            $datas = array(
            'result' => 1,
            'content' => $payout
    ...
    public function incoming_contact_message_notification()
    {
        $message = $this->contact_model->incoming_contact_message_notification_count();
        if(!empty($message)){
            $datas = array(
            'result' => 1,
            'content' => $message
