...
class Product_controller extends Home_Core_Controller
{
	...
	public function refund_request()
	{
		...
		$id = $this->input->post('refund_order_product_id', true);
		$data = [
			'refund_id' => uniqid(),
			'refund_track_code' => $code,
			'refund_order_product_id' => intval($id),
            'created_at' => date('Y-m-d H:i:s'),
		];
		
		$row = $this->db->get_where('refunds', ['refund_order_product_id' => intval($id)])->result();

        $order_product_row = $this->db->get_where('order_products', ['id' => intval($id)])->result()[0];
		
		if (count($row) > 0) {
			$this->session->set_flashdata('error', trans("refund_request_error"));
			redirect($this->agent->referrer());
		}
		

        if ($order_product_row->order_status == "completed" || $order_product_row->order_status == "shipped") {
            $updated = $order_product_row->updated_at;

            $now = time(); // or your date as well
            $your_date = strtotime($updated);
            $datediff = $now - $your_date;
            $_days = round($datediff / (60 * 60 * 24));

            $_refundLimit = $this->settings_model->general_settings->refund_period;


            if ($_days > $_refundLimit) {
                $this->session->set_flashdata('error', trans("refund_request_time_error"));
                redirect($this->agent->referrer());
            }
            ...
            $this->db->where('id', $id);
            $this->db->update('order_products', ['order_status' => 'refund_pending']);

            $sellerBalance = $this->db->get_where('users', ['id' => intval($order_product_row->seller_id)])->result()[0]->balance;

            $this->db->where('id',$order_product_row->seller_id);
            $this->db->update('users', ['balance' => $sellerBalance - $order_product_row->product_total_price]);

            $this->session->set_flashdata('success', trans("refund_request_created"));


            $seller = $this->db->get_where('users', ['id' => $order_product_row->seller_id])->result()[0];
            ...
            $data_array = array(
                ...
                'order_id' => $order_product_row->order_id,
            );
            $template = $this->get_email_templates($this->selected_lang->id,'product_refund_request');
            $this->load->model('email_model');

            if(!empty($template)){
                $send_mail            = [];
                $send_mail['to']      = $seller->email;
		...
		$this->session->set_flashdata('error', trans("cannot_refund_this_product"));
                redirect($this->agent->referrer());
	}
     ...
     * Add Product
     */
    public function add_product_user()
    {
        //check auth
        if (!$this->auth_check) {
            redirect(lang_base_url());
    ...
    public function add_product_post()
    {
        ...
        /* Check No Purchase Option  */
        $inputs = $this->input->post();
        if($inputs['listing_type'] == 'sell_on_site'){
            foreach ($inputs as $key => $value) {
                if (strpos($key, 'category_id_') !== false) {
                    $check_no_purchase = check_no_purchase_cat($value);
                    if($check_no_purchase){
                        $this->session->set_flashdata('error',trans('no_purchase_error'));
                        // redirect($this->agent->referrer());
                    }
                }
            }
    ...
    public function add_product_post_user()
    {
        //check auth
        if (!$this->auth_check) {
            redirect(lang_base_url());
        ...
        if ($this->product_model->add_product()) {
            ...
            redirect(generate_url("sell_now", "product_details_user") . '/' . $last_id);
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
            redirect($this->agent->referrer());
        }
     ...
     */
    public function edit_product_details_user($id)
    {
        //check auth
        if (!$this->auth_check) {
            redirect(lang_base_url());
        ...
        // $this->load->view('admin/includes/_header', $data);
        // $this->load->view('admin/product/edit_product_details');
        // $this->load->view('admin/includes/_footer');
    }
