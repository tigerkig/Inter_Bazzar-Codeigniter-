...
class Home_controller extends Home_Core_Controller
{
    ...
    {
        /*
		$getloc = json_decode(file_get_contents("http://ip-api.com/json"), true);
		$country = $getloc['country'];
		
		$country = $this->location_model->get_country_by_name($country);
		
		$this->session->set_userdata('mds_default_location_id', $country->id);
*/
    ...
    public function product($slug)
    {
        ...
        if (empty($data['product'])) {
            ...
            $categories_array = get_parent_categories_array($data["product"]->category_id);
            $is_allow_order = 1;
            if (!empty($categories_array)) {
                foreach ($categories_array as $item_array) {
                    $item_category = $this->category_model->get_category($item_array->id);
                    if (!empty($item_category) && $item_category->is_allow_order == 0) {
                        $is_allow_order = 0;
                    }
                }
            }
            $data["category_is_allow_order"] = $is_allow_order;
            ...
            if($is_allow_order == 0){
                $data["product"]->listing_type = 'ordinary_listing';
            }
            $this->load->view('partials/_header', $data);
    ...
    */
    ...
    // Author : 225
    // To set location by browser location
    public function set_default_location_get()
    {
