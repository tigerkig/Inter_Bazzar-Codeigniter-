...
class Earnings_admin_controller extends Admin_Core_Controller
{
    ...
    public function payout_requests()
    {
        ...
        $data['panel_settings'] = $this->settings_model->get_panel_settings();
        $data['payout_settings'] = $this->settings_model->get_payout_settings_data();
    ...
    /* Author : 225 */
    public function add_payout_setting_post(){

        if ($this->settings_model->add_payout_setting()) {
            $this->session->set_flashdata('success', trans("msg_updated"));
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
        }
    ...
    public function update_payout_setting_post(){
        $id = $this->input->post('id', true);
        if ($this->settings_model->update_payout_setting($id)) {
            $this->session->set_flashdata('msg_success', 1);
            $this->session->set_flashdata('success', trans("msg_updated"));
        } else {
            $this->session->set_flashdata('msg_success', 1);
            $this->session->set_flashdata('error', trans("msg_error"));
    ...
    public function delete_payout_setting_get(){
        $id = $this->input->get('id', true);
        if ($this->settings_model->delete_payout_setting($id)) {
            $data = ['status' => true];
        } else {
            $data = ['status' => false];
        }
    ...
    public function add_payout_account(){
        if ($this->settings_model->add_payout_account_model()) {
            $this->session->set_flashdata('msg_success', 1);
            $this->session->set_flashdata('success', trans("msg_updated"));
        } else {
            $this->session->set_flashdata('msg_success', 1);
