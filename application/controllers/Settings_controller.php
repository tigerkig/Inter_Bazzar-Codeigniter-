...
class Settings_controller extends Admin_Core_Controller
{
    ...
    */
    ...
    {
        if ($this->settings_model->update_location_settings()) {
            $this->session->set_flashdata('success', trans("msg_updated"));
            $this->session->set_flashdata("mes_pay", 1);
            redirect($this->agent->referrer());
        } else {
    ...
    /**
     * Payby.Me Settings Post
     */
    public function paybyme_settings_post()
    {   
        if ($this->settings_model->update_paybyme_settings()) {
            $this->session->set_flashdata('success', trans("msg_updated"));
            $this->session->set_flashdata("paybyme_paypal", 1);
            redirect($this->agent->referrer());
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
            $this->session->set_flashdata("paybyme_paypal", 1);
     ...
     * moka Settings Post
     */
    public function moka_settings_post()
    {
        if ($this->settings_model->update_moka_settings()) {
            $this->session->set_flashdata('success', trans("msg_updated"));
            $this->session->set_flashdata("mes_moka", 1);
            redirect($this->agent->referrer());
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
            $this->session->set_flashdata("mes_moka", 1);
