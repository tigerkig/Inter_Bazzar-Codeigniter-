...
class Admin_controller extends Admin_Core_Controller
{
    ...
    public function index()
    {
        ...
        $data['latest_orders'] = $this->order_admin_model->get_orders_limited(100);
        $data['latest_order_products'] = $this->order_admin_model->get_order_products_status(100);
    ...
    */
    ...
    /*
    * Multiple Currency Settings Post
    */
    public function multiple_currency_settings_post()
    {   
        if ($this->currency_model->update_multiple_currency_settings()) {
            $this->session->set_flashdata('multi_msg_settings', 1);
            $this->session->set_flashdata('success', trans("msg_updated"));
        } else {
            $this->session->set_flashdata('multi_msg_settings', 1);
            $this->session->set_flashdata('error', trans("msg_error"));
    ...
    */
    ...
    public function system_settings_post()
    {
        ...
        if ($this->settings_model->update_system_settings()) {
            $this->session->set_flashdata('system_setting', 1);
            $this->session->set_flashdata('success', trans("msg_updated"));
            redirect($this->agent->referrer());
        } else {
            $this->session->set_flashdata('system_setting', 1);
    ...
    /* Security Setting POST */
    public function security_settings_post(){
        if ($this->settings_model->update_security_settings()) {
            $this->session->set_flashdata('security_setting', 1);
            $this->session->set_flashdata('success', trans("msg_updated"));
            redirect($this->agent->referrer());
        } else {
            $this->session->set_flashdata('security_setting', 1);
    ...
    */
    ...
    public function approve_shop_opening_request()
    {
        ...
        if ($this->auth_model->approve_shop_opening_request($user_id)) {
            ...
            $accept_reject = trans('approve');
            $email_button_text = trans("start_selling");
            if ($submit == 0) {
                $accept_reject = trans('decline');
            ...
            $this->approve_reject_shop_request_mail($user,$accept_reject);
    ...
    public function preferences()
    {
		...
		$data['categories'] = $this->db->query('SELECT slug, id from categories ORDER BY category_order')->result();
    ...
    {
        $data['title'] = trans("import_export");
      ...
      /*  $query = $this->db->query("SELECT * FROM db_backup");
       // return $query->result();
        ...
        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/import_export', $data);
        $this->load->view('admin/includes/_footer');
	...
	public function exportDB(){ 
            ...
            $prefs = array(     
				'tables'        => array_keys($this->input->post('table')),
                'format'      => 'txt',             
            ...
            $db_name = 'db_'. date("Y-m-d-H-i-s") .'.sql';
            /* insrt */
            ...
            $this->db->insert('db_backup',$data);
            /* end insert*/
            $save = 'backups/database/'. $db_name;
    ...
    /*end download */
    public function download_db($fileName = NULL) {   
       if ($fileName) {
        $file = 'backups/database/'. $fileName;
        // check file exists    
        echo $fileName;
       // die('ssss');
        if (file_exists ( $file )) {
    ...
    */
    ...
    public function get_currency_exchange_rate_post($base_price,$base_currency,$admin_currency){
        $data = ['status' => 0];
        echo json_encode($data);
    }

    /* Approve Reject Shop Open Request */
    public function approve_reject_shop_request_mail($user,$content){

        $data_array = array(
            'user' => $user->first_name,
            'email' => $user->email,
            'accept_reject' => $content,
        );
        $this->load->model('order_model');
        
        $template = $this->order_model->get_email_templates($this->selected_lang->id,'shop_open_close_request');
    ...
    public function send_text_email_get(){
        $this->order_model->send_order_status_update(3);
        exit;
        $data_array = array(
            'user' => 'Sawan',
            'email' => 'sawanshakya25@gmail.com',
            'verification_url' => base_url(),
            'soft_name' => 'Sawan Shakya'
        );
        $this->db->select('*');
        $this->db->where('name','membership_registeration');
        $this->db->where('email_templates_messages.language_id','2');
        $this->db->join('email_templates_messages','email_templates_messages.email_templates_id = email_templates.id','LEFT JOIN');
        $resp = $this->db->get('email_templates')->row();
        $message = $resp->message;
        foreach ($data_array as $key => $value) {
            $message = str_replace('{'.$key.'}', $value, $message);
        }
        // $message = str_replace('{email}', $data_array['email'], $message);
        // $message = str_replace('{verification_url}', $data_array['verification_url'], $message);
        // $message = str_replace('{soft_name}', $data_array['soft_name'], $message);
        // echo $message;
        // exit;

        $message = str_replace('<meta name="copied-from" content="wysihtml5" />', ' ', $message);
        $data = array(
            'to' => 'sawanshakya1995@gmail.com',
            'subject' => $resp->subject,
            'message' => $message
        );

        $this->load->model('email_model');
        echo $this->email_model->send_email($data);
        // print_r($resp);
        exit;
    }
}
