...
if (!function_exists('get_logo_mobile')) {
	function get_logo_mobile($settings)
    {
        if (!empty($settings)) {
            if (!empty($settings->logo_mobile) && file_exists(FCPATH . $settings->logo_mobile)) {
                return base_url() . $settings->logo_mobile;
            }
        }
        return base_url() . "assets/img/logo_mobile.svg";
    }
}
...
if (!function_exists('get_billing_country')) {
    function get_billing_country($id)
    {
        $ci =& get_instance();
        return $ci->location_model->get_billing_country($id);
    }
}
...
// Payouts Withdrawn Days
if (!function_exists('payout_withdrawn_remaining_day')) {
    function payout_withdrawn_remaining_day($string="")
    {
        $ci =& get_instance();
        $Withdrawn_days = $ci->general_settings->commission_withdrawn_days;
        $resp = $ci->earnings_model->last_withdrawn_date($string);

        if(!empty($resp)){
            $date1 = new DateTime($resp); 
            $date2 = new DateTime(date('Y-m-d')); 
            $diff  = $date2->diff($date1)->format('%a');;
            $remaining_days = ($Withdrawn_days - $diff);
...
if (!function_exists('get_country_id')) {
    function get_country_id($name="")
    {
        $ci =& get_instance();
        $Withdrawn_days = $ci->general_settings->commission_withdrawn_days;
        $resp = $ci->location_model->search_countries($name);
...
if (!function_exists('get_payout_name')) {
    function get_payout_name($id=""){
        $ci =& get_instance();
        $ci->db->select('name');
        $ci->db->where('id',$id);
        $resp = $ci->db->get('payout_settings')->row();
        if($resp){
            return $resp->name;
        }
        return false;
...
if (!function_exists('email_template_replace')) {
    function email_template_replace($data_array="",$template=""){
...
if (!function_exists('email_subject_replace')) {
    function email_subject_replace($data_array="",$subject=""){
        $message = $subject;
        if(!empty($data_array)){
            foreach ($data_array as $key => $value) {
                $message = str_replace('{'.$key.'}', $value, $message);
...
if (!function_exists('get_shop_request')) {
    function get_shop_request(){
        $number = 0;
        $ci = &get_instance();
        $ci->db->select('count(id) AS total');
        $ci->db->where('is_active_shop_request',1);
        $resp = $ci->db->get('users')->row();
        if($resp){
            $number = $resp->total;
        }
        return $number;
...
if (!function_exists('get_payout_details')) {
    function get_payout_details($payout_id,$user_id,$column_name){
        $ci = &get_instance();
        $ci->db->select($column_name);
        $ci->db->where('payout_id',$payout_id);
        $ci->db->where('user_id',$user_id);
        $resp = $ci->db->get('user_other_payout_accounts')->row();
        if($resp){
            return $resp->$column_name;
        }
        return false;
    }
}

if (!function_exists('get_payout_request_details')) {
    function get_payout_request_details($payout_name,$user_id,$column_name){
        $ci = &get_instance();
        $ci->db->select($column_name);
        $ci->db->where('user_id',$user_id);
        $ci->db->where('payout_name',$payout_name);
        $resp = $ci->db->get('user_other_payout_accounts')->row();
        if($resp){
            return $resp->$column_name;
        }
