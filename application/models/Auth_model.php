...
class Auth_model extends CI_Model
{
    ...
    public function send_email_activation($user_id, $token)
    {
        if (!empty($user_id)) {
            ...
            if (!empty($user)) {
                ...
                $this->auth_model->send_verification_mail($user,$data);
    ...
    public function filter_mem($search)
    {
        // echo $search;
        $this->db->where('role', "member")
    ...
    public function ban_remove_ban_user($id)
    {
        ...
        if (!empty($user)) {
            ...
            if ($user->banned == 0) {
                $data['banned'] = 1;
                $ban_status = trans('banned');
            }
            if ($user->banned == 1) {
                $ban_status = trans('active');
                $data['banned'] = 0;
            }
            $this->ban_unbanned_user_shop($user,$ban_status);
            $this->db->where('id', $id);
            return $this->db->update('users', $data);
        }
    ...
    public function open_close_user_shop($id)
    {
        ...
        if (!empty($user)) {
            ...
            if ($user->role == 'member') {
                $data['role'] = 'vendor';
                $open_close = trans('open');
            } else {
                $open_close = trans('closed');
                $data['role'] = 'member';
            }
            
            $this->open_close_shop_email($user,$open_close);
    ...
    public function send_verification_mail($user,$data){
        
        $this->load->model("order_model");

        $template = $this->order_model->get_email_templates($this->selected_lang->id,'email_verification');
        
        $data_array = array(
            'user' => $user->first_name,
            'email' => $user->email,
            'verification_url' => lang_base_url() . "confirm?token=" . $data['token'],
            'soft_name' => 'InterBazaar'
        );
        if(!empty($template)){
            $send_mail            = [];
    ...
    /* Open / Close Shop Mail Template */
    public function open_close_shop_email($user,$opn_close){
        $this->load->model("order_model");

        $template = $this->order_model->get_email_templates($this->selected_lang->id,'shop_open_close_user');
        
        $data_array = array(
            'user' => $user->first_name.' '.$user->last_name,
            'email' => $user->email,
            'shop_status' => $opn_close,
        );

        if(!empty($template)){
            $send_mail            = [];
            $send_mail['to']      = $user->email;// $user->email;
            $send_mail['subject'] = $template->subject;
            $send_mail['message'] = email_template_replace($data_array,$template);
            $this->load->model("email_model");
            $this->email_model->send_email_mail_template($send_mail);
    ...
    /* Ban Unbanned User Shop */
    public function ban_unbanned_user_shop($user,$open_status){
        $this->load->model("order_model");

        $template = $this->order_model->get_email_templates($this->selected_lang->id,'ban_user');
        
        $data_array = array(
            'user' => $user->first_name.' '.$user->last_name,
            'email' => $user->email,
            'ban_status' => $open_status,
        );
