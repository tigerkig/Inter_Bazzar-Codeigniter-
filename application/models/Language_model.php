...
class Language_model extends CI_Model
{
    ...
    public function add_language()
    {
        ...
        if ($this->db->insert('languages', $data)) {
            ...
            $email_templates = $this->get_language_email_templates(1);
            if (!empty($email_templates)) {
                foreach ($email_templates as $template) {
                    $data_translation = array(
                        'language_id' => $language_id,
                        'email_templates_id' => $template->email_templates_id,
                        'subject' => $template->subject,
                        'message' => $template->message
                    );
                    $this->db->insert('email_templates_messages', $data_translation);
                }
            }
    ...
    public function delete_language($id)
    {
        ...
        if (!empty($language)) {
            ...
            $sql = "SELECT * FROM email_templates_messages WHERE language_id = ?";
            $tmp_query = $this->db->query($sql, array(clean_number($language->id)));
            $templates = $tmp_query->result();
            if (!empty($templates)) {
                foreach ($templates as $template) {
                    $this->db->where('id', $template->id);
                    $this->db->delete('email_templates_messages');
                }
            }
