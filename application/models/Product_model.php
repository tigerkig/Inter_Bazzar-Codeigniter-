...
class Product_model extends Core_Model
{
    ...
    public function query_string($type = "active", $return_count = false, $compile_query = true)
    {
        ...
        if ($return_count == true) {
            ...
            $select = "products.id, products.title, products.slug, products.product_type, products.listing_type, products.category_id,  products.price, products.currency, products.discount_rate, products.shipping_cost,
    ...
    {
        $sql = "SELECT products.*, categories.not_refundable as not_refundable, users.username as user_username, users.shop_name as shop_name, users.role as user_role, users.slug as user_slug FROM products 
    ...
    */
	...
	public function get_products_where($where)
    {
        $sql = $this->query_string(). " and ".$where;
        $query = $this->db->query($sql);
        return $query->result();
    }
