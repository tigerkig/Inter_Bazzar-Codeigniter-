...
class Order_admin_model extends CI_Model
{
    ...
    public function update_order_payment_received($order_id)
    {
        ...
        if (!empty($order)) {
            ...
            if (!empty($order_products)) {
                ...
                $this->order_model->update_order_payment_status($order_id);
            }
        }
    }
    ...
    public function update_order_product_status($order_product_id)
    {
        ...
        if($order_status != $order_product->order_status){
            if (!empty($order_product)) {
                ...
                if($this->db->update('order_products', $data)){
                    ...
                    $this->order_model->send_order_status_update($order_product_id);
                    return true;
                }
            }
        }
    ...
    {
        $this->db->select('order_products.*, refunds.*');
     ...
     public function get_paginated_refunds($per_page, $offset)
    {

        $this->db->select('orders.*','order_products.*, refunds.*');
        $this->db->join('order_products','refunds.refund_order_product_id = order_products.id');
        $this->db->join('orders','order_products.order_id = orders.id');
        $this->db->from('refunds');
        $this->db->where('order_products.order_status', 'refund_'.$this->input->get('type'));
    ...
    public function get_order_products_status($limit)
    {

        // $this->db->select('orders.* , order_products.*');
    ...
    // Order Notification
     public function get_orders_notification_count()
    {
    ...
    // Bank Transfer Notification
     public function bank_transfer_notification_count()
    {
        $this->db->where('status', 'pending');
        return $this->db->count_all_results('bank_transfers');
    }

     // Order Complete With Bank Notification
     public function order_complete_bank_notification_count()
    {
