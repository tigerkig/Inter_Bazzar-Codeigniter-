        ...
        if (!empty($cart)) {
            foreach ($cart as $item) {
                ...
                if ($item->purchase_type == 'bidding') {
                    ...
                    $cart_total->subtotal += currency_convert($object->price_calculated,$product->currency) * $item->quantity;

                    $cart_total->shipping_cost += currency_convert($item->shipping_cost,$product->currency);
            ...
            if (!empty($product)) {
                ...
                if ($cart_item->purchase_type == 'bidding') {
                    ...
                    if (!empty($quote_request) && $quote_request->status == 'pending_payment') {
						...
						$item->isnot_refundable = intval($product->not_refundable);
                        $item->is_stock_available = 1;
                        if ($this->form_settings->shipping != 1) {
...
public function add_to_cart_d(&$cart, $product, $quantity)
    {
        $appended_variations = $this->get_selected_variations($product->id)->str;
        $options_array = $this->get_selected_variations($product->id)->options_array;
        $object = $this->get_product_price_and_stock($product, $options_array, $quantity);
	...
	public function get_sess_cart_items_d($cart)
    {
        $new_cart = array();
        foreach ($cart as $cart_item) {
            $product = $this->product_model->get_available_product($cart_item->product_id);
	...
	public function calculate_cart_total_d($cart)
    {
        $cart_total = new stdClass();
        $cart_total->subtotal = 0;
        $cart_total->vat = 0;
	...
	public function check_cart_has_physical_product_d($cart_items)
    {
        if (!empty($cart_items)) {
            foreach ($cart_items as $cart_item) {
                if ($cart_item->product_type == 'physical') {
    ...
    //check cart has digital products
    public function check_cart_has_digital_product_d($cart_items)
    {
        if (!empty($cart_items)) {
            foreach ($cart_items as $cart_item) {
                if ($cart_item->product_type == 'digital') {
	...
	public function set_sess_cart_shipping_address_d($row)
    {
        $std = new stdClass();
	    ...
	    public function get_sess_cart_shipping_address_d($cart)
    {
