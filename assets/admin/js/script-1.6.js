...
$(document).ready(function(){
    $.ajax({
        type: "post",
         url: base_url + "ajax_controller/get_order_notification",
        data: "data",
        dataType: "json",
        success: function (response) {
            console.log(response.content);
            if(response.count > 0){
                $('#notification_badge').text(response.content);
                $('#notification_badge').show();

                var html = '';
                $.each(response.data,function(index, el) {
                    html += '<p><a class="dropdown-item" style="white-space: normal !important; padding-top: 10px; display:inline-block; padding-bottom: 10px; border-bottom:1px solid #eee;color:black;" href='+"<?php echo base_url('admin/read/');?>"+el.id+'><small class="form-text text-muted"><strong>'+el.title+'</strong>: '+el.message+'<br>'+el.created_at+'</small></a></p>';
                });
                $('#put_notification_message').html(html);

            }else{
                $('#notification_badge').html('<div class="text-center">0</div>');
            }
        }
    });
});
